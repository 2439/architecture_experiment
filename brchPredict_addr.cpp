...
class SaturatingCnt
{
    ...
    public:
        ...
        void reset() { val = init; valid = false; targetAddr = 0; }
...
class BranchPredictor
{
    public:
        BranchPredictor() { }
        virtual PredictResult predict(ADDRINT addr) { return FALSE; };
        virtual void update(BOOL takenActually, PredictResult takenPredicted, ADDRINT addr) {};
};
...
class BHTPredictor: public BranchPredictor
{
    ...
    public:
        ...
        PredictResult predict(ADDRINT addr)
        {
            ...
            if(counter[Tag].getValid())
            {

            }
            ret.targetAddr = false
            return ;
        }
...
void Instruction(INS ins, void * v)
{
    if (INS_IsControlFlow(ins) && INS_HasFallThrough(ins))
    {
        INS_InsertCall(ins, IPOINT_TAKEN_BRANCH, (AFUNPTR)predictBranch,
                        IARG_INST_PTR, IARG_BOOL, IARG_BRANCH_TARGET_ADDR, TRUE, IARG_END);

        INS_InsertCall(ins, IPOINT_AFTER, (AFUNPTR)predictBranch,
                        IARG_INST_PTR, IARG_BOOL, IARG_BRANCH_TARGET_ADDR, FALSE, IARG_END);
    }
}
