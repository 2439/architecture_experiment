...
VOID PIN_FAST_ANALYSIS_CALL updateInsDependDistance(VOID *v)
{
	...
	vector<Registers*> regsGroup = *((vector<Registers*>*) v);
	...
	*/
	...
	for(vector<Registers*>::iterator it = regsGroup.begin(); it != regsGroup.end(); it++)
	{
		++insPointer;
...
VOID Instruction(TRACE trace, VOID *v)
{
	...
	for (BBL bbl = TRACE_BblHead(trace); BBL_Valid(bbl); bbl = BBL_Next(bbl))
	{
		...
		for(INS ins = BBL_InsHead(bbl); ins != BBL_InsTail(bbl); ins = INS_Next(ins))
		{
			...
			regsGroup.push_back(regs);
		}
		BBL_InsertCall(bbl, IPOINT_ANYWHERE, (AFUNPTR)updateInsDependDistance,IARG_FAST_ANALYSIS_CALL, IARG_PTR, (void*)&regsGroup, IARG_END);
	}
}
